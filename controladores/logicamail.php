...
function updateUser($token, $tiempo_vida, $User_Id)
{
    $conex = conexion();
    $Valor = 1;
    $sql = "UPDATE usuario SET request_password=:request_pass, token_password=:token_pass, expired_session=:expired WHERE usuario_id= :id_usuario";
    try {
        $stmt = $conex->prepare($sql);

        $stmt->bindParam(":request_pass", $Valor);
        $stmt->bindParam(":token_pass", $token);
        $stmt->bindParam(":expired", $tiempo_vida);
...
function EnviarCorreoResetPassword($Correo, $NombreReceptor, $userid, $token_User)
{
    ...
    try {
        ...
        <a href="http://localhost/Animcomic-VersionUltima/cambiar_password.php?id='.$userid.'&&token='.$token_User.'">cambiar password aqui</a></b>';
